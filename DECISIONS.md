...
## 2026-09-12, hero glass + Live Web Photos + style library
- 2026-09-12 | Hero glass now DARKENS its backdrop instead of lifting it | Jon: "some of the blue pages are too bright blue." The first pass used brightness(1.2)/saturate(260%), which turned the blue heroes electric. Now a black tint plus brightness(.84) and saturate(155%): hot areas calm down, white copy gains contrast, and the motion still reads through at blur(14px).
- 2026-09-12 | Hero panel padding cut roughly in half and the interior stack tightened | Jon: "tighter around the words, less white space inside the cards." Measured 102px of the services hero's 368px height as padding. Now 27px block / 28px inline, driven by shared `--hero-glass-*` tokens so both hero rules stay in step.
- 2026-09-12 | Live Web Photos links out to https://livewebphotos.com in a new tab everywhere; the local /ecosystem/live-web-photos page is deleted | Jon's instruction. Footer gained external-link handling (target/rel) rather than hardcoding one case.
- 2026-09-12 | Added a 301! from /ecosystem/live-web-photos to the external domain rather than letting it 404 | That URL is in the live sitemap, so it is indexed. The redirect preserves it.
- 2026-09-12 | Rebuilt /services/custom-styles-and-carousel-cards as a filtered portfolio with all 88 images and the 3 motion clips | Jon: "bring over the cards and the styles, make it a filtered portfolio page with those 5 styles." Reverses an earlier v3 session that dropped every image under a no-image rule and left text cards.
- 2026-09-12 | Grids are generated from the filename pattern, not hand-listed | Files are perfectly regular (`<style>-NN.jpg`, `cards/<style>-card-<cat>-N.jpg`), so 88 paths come from a 5-line data table. No transcription risk.
- 2026-09-12 | Real pixel dimensions generated into src/data/style-media.json | The sources are five different shapes (1194x1600, 1600x1600, 1600x893, 1073x1600, 1600x1194), so width/height could not be hardcoded. The tile owns a 3:4 ratio and the image covers it, which keeps the grid uniform.
- 2026-09-12 | Lightbox open state is a class, not the `hidden` attribute | `.lb{display:grid}` outranks the UA's `[hidden]{display:none}`, and the scoped override was dropped from the emitted CSS, so the overlay was open on page load.
- 2026-09-12 | Motion clips attach their src on panel activation, preload metadata, and carry controls | Lazy attach keeps 26MB of video off every other page load; the metadata preload paints a first frame, and the controls are the fallback where autoplay is refused.
